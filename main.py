...
NAME = ['KNN', 'GaussianNB', 'SVM', 'LRC',   'DecisionTree']
RESULTS = []
# disable warnings
...
df = pd.read_csv('dataset_/Iris.csv')
st.write(df)
...
# train test split
    ...
    if st.sidebar.checkbox('Algorithms'):
        ...
        if st.checkbox('KNN'):
            ...
            kfold_knn = model_selection.KFold(n_splits=10, shuffle=True,  random_state=7)
            knn_result = model_selection.cross_val_score(knn, X, y, cv=kfold_knn, scoring='accuracy')
        ...
        # Gaussian Naive Baise Algorithm
        if st.checkbox('GaussianNB'):
            gnb = GaussianNB()
            # start train
            kfold_gnb = model_selection.KFold(n_splits=10, shuffle=True, random_state=7)
            gnb_result = model_selection.cross_val_score(
                gnb, X, y, cv=kfold_gnb, scoring='accuracy'
            )
            SCORE['GaussianNB'].append(gnb_result)
        ...
        if st.checkbox('SVC'):
            ...
            kfold_svc = model_selection.KFold(n_splits=10, shuffle=True, random_state=7)
        ...
        if st.checkbox('Logistic Regression'):
            ...
            kfold_lrc = model_selection.KFold(n_splits=10, shuffle=True, random_state=7)
            ...
            st.write('classifier name: Logistic Regression')
        ...
        if st.checkbox('Decision Tree Classifier'):
            ...
            kfold_dt = model_selection.KFold(n_splits=10, shuffle=True, random_state=7)
    ...
    if st.sidebar.checkbox('Results'):
        ...
        if not SCORE:
            ...
            st.write('Gaussian accuracy score: {} '.format(
                round(SCORE['GaussianNB'][0].mean(), 2)))
